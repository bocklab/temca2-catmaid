...
class StreamWrapper:
    ...
    def __init__(self, stream, total_size=0):
        self.stream = stream
...
def get_archive(url, path, checksum=None, dryrun=False):
    print("Downloading %s" % url)
    r = requests.get(url, stream=True)
    total_size = int(r.headers['content-length'])
    stream = StreamWrapper(r.raw, total_size=total_size)
    ...
    if stream.gethash().hexdigest() != checksum:
        ...
        print("Hashes match (%s=%s)" % (stream.gethash().hexdigest(), checksum))
...
def main():
    ...
    for layer in range(args.first, args.last+1):
        ...
        get_archive(layer_url, args.datapath, checksum=checksums[filename], dryrun=args.dryrun)
